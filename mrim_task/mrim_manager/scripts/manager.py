...
class Evaluator:
    ...
    def inspectionPointToViewPoint(self, inspection_point):
        x = inspection_point.position.x + self.viewpoints_t_distance * np.cos(inspection_point.inspect_heading) * np.sin(inspection_point.inspect_tilt)
...
class MrimManager:
    ...
    def __init__(self):
        ...
        constraint_manager = "mrs_uav_managers/constraint_manager/"
        ...
        h_jerk = rospy.get_param('~' + constraint_manager + constraint_type + '/horizontal/jerk')
        h_snap = rospy.get_param('~' + constraint_manager + constraint_type + '/horizontal/snap')

        va_speed = rospy.get_param('~dynamic_constraints/max_velocity/z')
        va_acc = rospy.get_param('~dynamic_constraints/max_acceleration/z')
        va_jerk = rospy.get_param('~' + constraint_manager + constraint_type + '/vertical/ascending/jerk')
        va_snap = rospy.get_param('~' + constraint_manager + constraint_type + '/vertical/ascending/snap')

        vd_speed = rospy.get_param('~dynamic_constraints/max_velocity/z')
        vd_acc = rospy.get_param('~dynamic_constraints/max_acceleration/z')
        vd_jerk = rospy.get_param('~' + constraint_manager + constraint_type + '/vertical/descending/jerk')
        vd_snap = rospy.get_param('~' + constraint_manager + constraint_type + '/vertical/descending/snap')

        heading_speed = rospy.get_param('~dynamic_constraints/max_heading_rate')
        heading_acc = rospy.get_param('~dynamic_constraints/max_heading_rate_acceleration')
        heading_jerk = rospy.get_param('~' + constraint_manager + constraint_type + '/heading/jerk')
        heading_snap = rospy.get_param('~' + constraint_manager + constraint_type + '/heading/snap')
        ...
        pub_mesh = rospy.Publisher('mesh_out', Marker, queue_size=1)
        ...
        self.evaluator_ = Evaluator(self.inspection_problem, viewpoint_distance, viewpoint_distance,
        ...
        self.visualizer_ = Visualizer(self.inspection_problem, jsk_diagnostics_publishers, cones_publishers,\
                                      pub_inspection_points, pub_viewpoints, pub_collisions, pub_start_positions, pub_start_arrows, pub_safety_area, pub_cloud, pub_mesh,\
        ...
        self.visualizer_.setFov1(fov_length_1, horizontal_aov_1, vertical_aov_1, 0.05, 1.0, 0.0, 0.0)
        self.visualizer_.setFov2(fov_length_2, horizontal_aov_2, vertical_aov_2, 0.05, 0.0, 1.0, 1.0)
        ...
        if run_type == 'offline':
        ...
        elif run_type == 'simulation' or run_type == 'realworld':
            ...
            if overall_status or (not run_type == 'realworld' and flight_always_allowed):
    ...
    def checkDynamicConstraints(self, trajectories, constraints):
        ...
        for k in range(len(trajectories)):
            ...
            translation_constraints_check_successful = vel_x_ok and vel_y_ok and vel_asc_ok and vel_desc_ok and vel_heading_ok and  acc_x_ok and acc_y_ok and acc_asc_ok and acc_desc_ok and acc_heading_ok
    ...
    def runSimulationMonitoring(self, trajectories, minimum_obstacle_distance, minimum_mutual_distance, dynamic_constraints_ok_list):
        ...
        rate_mission_check = rospy.Rate(20.0)
