...
#        self.fov1_rgb = self.createConeMsg(fov_length, aov_h, aov_v, x_scale, red, green, blue, 2, "RGB")
        self.fov1_rgb = self.createConeMsg(fov_length, aov_h, aov_v, x_scale, red, green, blue, 2, "Thermal")
    ...
    def createViewPointsMsg(self, inspection_points, viewpoints, scale):
        ...
        red = ColorRGBA(1.0, 0.4, 0.27, 0.7)
        green = ColorRGBA(0.0, 1.0, 0.0, 0.7)
        blue = ColorRGBA(0.0, 1.0, 1.0, 0.7)
        purple = ColorRGBA(1.0, 0.0, 1.0, 0.7)
    ...
    def createStartArrowsMsg(self, trajectories):
        ...
        for k in range(len(trajectories)):
            ...
            marker.frame_locked    = True
            marker.pose.orientation = Quaternion(0.0, 0.0, 0.0, 1.0)
            marker.color = ColorRGBA(1.0, 0.4, 0.27, 0.5) if k == 0 else ColorRGBA(0.0, 1.0, 1.0, 0.5)
    ...
    def createInspectionPointsMsg(self, inspection_points, viewpoints, scale):
        ...
        marker.frame_locked    = True
        marker.pose.orientation = Quaternion(0.0, 0.0, 0.0, 1.0)
        red = ColorRGBA(1.0, 0.4, 0.28, 0.7)
        green = ColorRGBA(0.0, 1.0, 0.0, 0.7)
        blue = ColorRGBA(0.0, 1.0, 1.0, 0.7)
        purple = ColorRGBA(1.0, 0.0, 1.0, 0.7)
